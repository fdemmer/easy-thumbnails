...
from easy_thumbnails import VIL, alias, storage
...
class Base(test.BaseTest):
        ...
        context = Context(
            {
                ...
                'invalid_filename': f'not-{self.filename}',
                'size': (90, 100),
    ...
    def verify_thumbnail(
        ...
        self.assertTrue(
            ...
            f'Thumbnail file {expected_filename!r} not found',
        )
...
class ThumbnailerFilterTest(ThumbnailerBase):
    ...
    def test_relative_name(self):
        src = '{% with t=storage|thumbnailer:filename %}{{ t.small.url }}{% endwith %}'
        output = self.render_template(src)
        expected = self.verify_thumbnail(
            (20, 20), settings.THUMBNAIL_ALIASES['']['small']
        )
    ...
    def test_invalid(self):
        src = '{% with t=invalid_filename|thumbnailer %}{{ t.small.url }}{% endwith %}'
        output = self.render_template(src)
...
class ThumbnailerPassiveFilterTest(ThumbnailerBase):
    def test_check_generate(self):
        src = '{% with t=filename|thumbnailer_passive %}{{ t.generate }}{% endwith %}'
        output = self.render_template(src)
...
class ThumbnailSVGImage(test.BaseTest):
    ...
    def test_named_file(self):
        ...
        expected = (
            ...
            ' translate(0,-30)" clip="0 0 30 30"/></svg>'
        )
