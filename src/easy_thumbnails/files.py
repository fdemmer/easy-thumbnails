...
class ThumbnailFile(ImageFieldFile):
    ...
    def tag(self, alt='', use_size=None, **attrs):
        ...
        attrs = ' '.join(
            [f'{key}="{escape(value)}"' for key, value in sorted(attrs.items())]
        )
...
class Thumbnailer(File):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for default in (
            ...
            attr_name = f'thumbnail_{default}'
    ...
    def generate_thumbnail(self, thumbnail_options, silent_template_exception=False):
        ...
        if os.path.splitext(self.name)[1][1:].lower() == 'svg':
            ...
            img = engine.save_pil_image(
                ...
                keep_icc_profile=thumbnail_options.get('keep_icc_profile', False),
            )
        data = img.read()
    ...
    def get_thumbnail(
        self, thumbnail_options, save=True, generate=None, silent_template_exception=False
    ):
        """
        ...
        if not thumbnail:
            if generate:
                thumbnail = self.generate_thumbnail(
                    thumbnail_options, silent_template_exception=silent_template_exception
                )
                if save:
    ...
    def save_thumbnail(self, thumbnail):
        ...
        if settings.THUMBNAIL_CACHE_DIMENSIONS:
            dimensions_cache, created = models.ThumbnailDimensions.objects.get_or_create(
