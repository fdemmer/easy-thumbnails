        ...
        if isinstance(size, str):
            ...
            if m:
                ...
                if alias:
                    ...
                    if raise_errors:
                        raise TemplateSyntaxError(f'{size!r} is not a valid size.')
                    return self.bail_out(context)
...
def thumbnail(parser, token):
    ...
    **Debugging**
    ...
    if len(args) < 3:
        raise TemplateSyntaxError(
            f"Invalid syntax. Expected '{{% {tag} source size [option1 option2 ...] %}}'"
            f" or '{{% {tag} source size [option1 option2 ...] as variable %}}'"
        )
    ...
    for arg, value in args_list:
        if arg in VALID_OPTIONS:
            ...
            raise TemplateSyntaxError(f"'{tag}' tag received a bad argument: '{arg}'")
